...
import { getPool, waitForDatabase } from './db/pool';
...
app.use(
  ...
  cors({
    origin: (origin) => {
      ...
      const CORS_ORIGINS = env.BRAIN_CORS_ORIGINS.split(',')
        .map((o) => o.trim())
        .filter((o) => o.length > 0);
...
app.get('/health', async (c) => {
  ...
  const embeddingStatus =
    env.EMBEDDING_PROVIDER === 'local'
      ? 'local'
      : env.EMBEDDING_PROVIDER && env.EMBEDDING_PROVIDER !== 'local'
        ? 'vendor'
        : 'none';
...
async function bootstrap() {
  ...
  try {
    ...
    const targetDim =
      settingsResult.rows.length > 0 ? Number.parseInt(settingsResult.rows[0].value) : 384; // 默认 all-MiniLM-L6-v2 维度
    if (targetDim > 0) {
      const result = await ensureEmbeddingDimension(targetDim);
      if (result.migrated) {
        loggerInstance.info({ oldDim: result.oldDim, newDim: result.newDim }, '嵌入维度已自动迁移');
      }
    }
  } catch (err) {
