...
export default function StatusBar() {
  ...
  const statusConfig = {
    ok: {
      color: 'bg-green-500',
      label: t('status.ok', '服务正常'),
      Icon: CheckCircle,
      iconColor: 'text-green-500'
    },
    degraded: {
      color: 'bg-yellow-500',
      label: t('status.degraded', '服务降级'),
      Icon: Warning,
      iconColor: 'text-yellow-500'
    },
    error: {
      color: 'bg-red-500',
      label: t('status.error', '服务异常'),
      Icon: XCircle,
    ...
    <footer className="flex h-8 items-center justify-between border-t border-slate-200 bg-white px-4 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
      <div className="flex items-center gap-4">
        <span className="flex items-center gap-1.5">
          <span
            className={`h-2 w-2 rounded-full ${color} ${healthQuery.isFetching ? 'animate-pulse' : ''}`}
          ></span>
          <Icon
            size={12}
            className={statusConfig[status as keyof typeof statusConfig]?.iconColor}
          />
          {label}
        </span>
        {health?.lastSync && (
          <span
            ...
            title={new Date(health.lastSync!).toLocaleString()}
          >
            <Clock size={12} />
            {t('status.lastSync', '最后同步')}：{formatRelativeTime(health.lastSync)}
          </span>
        )}
        {health?.uptimeMs && (
          <span className="hidden sm:inline text-slate-400">
            {t('status.uptime', '运行 {{duration}}', { duration: formatDuration(health.uptimeMs) })}
          </span>
        )}
      </div>
      <div className="flex items-center gap-4">
